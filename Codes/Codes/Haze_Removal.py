...
hazy_images = load_images('path_to_hazy_images')
...
haze_model = build_unet(input_shape)
haze_model.compile(optimizer='adam', loss='mean_squared_error')
# Train the model


# Train the model
split = int(0.9 * len(hazy_images))
train_hazy, val_hazy = hazy_images[:split], hazy_images[split:]
train_clean, val_clean = clean_images[:split], clean_images[split:]
history = haze_model.fit(train_hazy, train_clean, epochs=50, batch_size=8, validation_data=(val_hazy, val_clean))


# Save the model
haze_model.save('haze_removal_model.h5')

# Predict and display results
predicted_images = haze_model.predict(val_hazy)
...
display_images(val_hazy, val_clean, predicted_images)